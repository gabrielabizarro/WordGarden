...
struct ContentView: View {
    ...
    private let wordsToGuess = ["RAT", "DOG", "CAT"] // All Caps!
    var body: some View {
        VStack {
            HStack{
    ...
    func updateGamePlay() {
        if !wordToGuess.contains(guessedLetter){
            guessesRemaining -= 1
            //Animate crumblin lead and play the incorrect sound
            imageName = "wilt\(guessesRemaining)"
            playSounds(soundName: "incorrect")
        ...
        if !revealedWord.contains("_") { // Guesssed when no "_" in revealedWord
            ...
            //playSounds(soundName: "word-guessed")
        } else if guessesRemaining == 0 { //Word Missed
            gameStatusMessage = "So Sorry, You're All Out of Guesses"
            wordsMissed += 1
            currentWordIndex += 11
            playAgainHidden = false
            //playSounds(soundName: "word-not-guessed")
        } else { //Keep Guessing
